...
public abstract class GSYBaseVideoPlayer extends FrameLayout implements GSYMediaPlayerListener {
    ...
    public GSYBaseVideoPlayer showSmallVideo(Point size, final boolean actionBar, final boolean statusBar) {
        ...
        try {
            ...
            mParams.x = 0;
            mParams.y = getStatusBarHeight(getContext());
            ...
            mParams.width =  getScreenWidth(getContext())/3;
            mParams.height = getScreenHeight(getContext())/3;
            //在小米2s手机上显示的浮窗宽比高小一半
            if (mParams.width < mParams.height){
                mParams.width = getScreenHeight(getContext())/2;
                mParams.height = getScreenHeight(getContext())/4;
            }
            GSYWindowManager.instance().addView(getContext(),gsyVideoPlayer, mParams);
            return gsyVideoPlayer;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void hideSmallVideo() {
        try {
            ...
            vp.addView(gsyVideoPlayer);
        } catch (Exception e) {
            e.printStackTrace();
        }
        GSYWindowManager.instance().remove();
    }
